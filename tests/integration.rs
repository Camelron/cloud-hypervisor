...
fn test_virtio_rtc() {
    ...
    let r = std::panic::catch_unwind(|| {
        ...
        // First, fail fast if any of the required
        // PTP kernel config options are missing.
        assert_eq!(
            guest
                .ssh_command(
                    "cat \"/boot/config-$(uname -r)\" | grep CONFIG_VIRTIO_RTC=y"
                )
                .unwrap()
                .trim(),
            "CONFIG_VIRTIO_RTC=y"
        ...
        assert_eq!(
            guest
                .ssh_command(
                    "cat \"/boot/config-$(uname -r)\" | grep CONFIG_VIRTIO_RTC_PTP=y"
                )
                .unwrap()
                .trim(),
        ...
        assert_eq!(
            guest
                .ssh_command(
                    "cat \"/boot/config-$(uname -r)\" | grep CONFIG_PTP_1588_CLOCK=y"
                )
                .unwrap()
                .trim(),
